...
function Home() {
	...
	useEffect(() => {
		...
		axios
			.get(`${process.env.REACT_APP_BACKEND_URL}/clubes?tipo=logo`)
			.then((response) => {
				setTorneos((prevTorneos) =>
					prevTorneos.map((torneo) => {
						const club = response.data.find((c) => c.id === torneo.clubVinculo);
						return { ...torneo, logo: club ? club.logo : null };
					})
				);
			})
			.catch((error) => console.error(error));
			...
			<div className='body_home'>
				<div className='desc_home'>
					...
					<p>
						...
						GolfPoint es más que una simple aplicación, es tu compañero ideal en el mundo del golf. Cuenta con una gestión integral de clubes y torneos,
						estadísticas individuales, con una interfaz simple e intuitiva, un entorno amigable y ágil para el registro, lectura y análisis de datos.
						Ofrece todo lo que necesitas para disfrutar al máximo tu pasión por el golf.
						<br />
